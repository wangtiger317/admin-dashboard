<template>
  ...
  >
    <template #default="{ hide }">
      ...
      >
        <h5 class="mb-0">Create Rekening Pulsa</h5>
        <feather-icon
      ...
      <validation-observer ref="refFormObserver">
        ...
        >
          <b-form-group label="Bank">
            <v-select
              v-model="form.idProviderNonBank"
          ...
          <b-form-group label="Name">
            <b-form-input
              id="basicInput"
              v-model="form.keteranganNonBank"
...
export default {
  ...
  computed: {
    options() {
      return this.$store.getters['services/getRekeningPulsa']
    },
  },
  async mounted() {
    await this.$store.dispatch('services/fetchRekeningPulsa')
  },
  methods: {
    ...
    async flipIn(variant, position, item) {
      ...
      if (action.value) {
        const { idProviderNonBank, ...data } = item
        const payload = {
          idProviderNonBank: idProviderNonBank.id,
          ...data,
        }
        if (
          await this.$store.dispatch('services/createRekeningPulsa', payload)
        ) {
