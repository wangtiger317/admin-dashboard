    ...
    <b-card no-body class="mb-0">
      <div class="m-2">
        <b-row>
          ...
          >
            <h2>Announcement Member</h2>
          </b-col>

          <!-- create Announcement Member THIS -->
      ...
      >
        ...
        <template #cell(actions)="{ item }">
          <div class="text-nowrap">
            ...
            <!-- Delete this -->
            <feather-icon
              @click="deleteMember(item, 'success', 'top-center')"
...
export default {
  ...
  computed: {
    ...mapGetters('broadcast', ['getMemberCast']),

    // broadcats total
    broadcastTotal() {
      const total = this.getMemberCast
      const arr = []
      let max = 0
      if (total) {
    ...
    // broadcats perpage
    broadcastPage() {
      const perpage = this.getMemberCast
      let perPage = 0
      if (perpage) {
  ...
  // test
  methods: {
    ...mapActions('broadcast', ['memberBrodacastList', 'updateMember']),
    async changeAdminPage(number) {
      const payload = {
        search: '',
        page: number,
      }
      await this.memberBrodacastList(payload)
    },
    ...
    async flipInSave(variant, position, item) {
      ...
      if (res.value) {
        if (await this.updateMember(item)) {
    ...
    // flipInDelete
    deleteMember(broadcast, variant, position) {
      this.$swal({
        title: 'Are you sure?',
        ...
        if (result.value) {
          this.$store.dispatch('broadcast/deleteMember', broadcast)
          // toast notification this
          this.$toast(
            {
              component: ToastificationContent,
              props: {
                title: 'Success',
                icon: 'CheckIcon',
                text: `Success Member was Deleted`,
                variant,
              },
            },
            {
