<template>
  <b-card-text>
    <b-card no-body class="mb-0 pb-2 mt-3">
      ...
      <b-table
        :fields="fields"
        :items="getbonusPotongan.setting"
        responsive
        show-empty
        empty-text="No Data"
    ...
    <b-card no-body class="mb-0 mt-4">
      <div class="m-2">
        <b-row>
          <b-col
            ...
            class="d-flex align-items-center justify-content-start"
          >
            <h2>List Bonus Casback</h2>
          </b-col>
        </b-row>
      </div>
      <b-table
        :fields="cashback"
        :items="getbonusPotongan.listBonus.data"
        responsive
        show-empty
        empty-text="No Data"
        class="position-relative"
      >
        ...
        <!-- max_kekalahan -->
        <template #cell(min_kekalahan)="{ item }">
          <b-form-group>
            <cleave
              id="number"
              v-model="item.max_lose"
              class="form-control"
              :raw="false"
        ...
        <!-- hadiah -->
        <template #cell(max_kekalahan)="{ item }">
          <b-form-group>
            <cleave
              id="number"
              v-model="item.min_lose"
              class="form-control"
              :raw="false"
              :options="options.number"
            />
        ...
        <template #cell(actions)="{ item }">
          <div class="text-nowrap">
            ...
            <feather-icon
              ...
              size="16"
              class="cursor-pointer mx-1"
              @click="updatePotongan(item, 'success', 'top-center')"
            />
            <b-tooltip
              title="Save"
              :target="`invoice-row-${item.id}-save-icon`"
...
export default {
  ...
  // computed
  computed: {
    ...mapGetters('bonus', ['getbonusPotongan']),
    allTotal() {
      const list = this.getbonusPotongan.listBonus
      const arr = []
      let max = 0
      if (list) {
        arr.push(list.total)
  ...
  // methods
  methods: {
    ...mapActions('bonus', ['fetchPotonganList', 'updateCronTurn']),

    // fetchDataList store
    async fetchDataList(payload) {
      await this.fetchPotonganList(payload)
    },
    ...
    // flipInSave Cashback
    updatePotongan(bonus, variant, position) {
      this.$swal({
        title: 'Are you sure?',
        ...
        if (result.value) {
          this.$store.dispatch('bonus/updatePotongan', bonus)
          this.$toast(
            {
