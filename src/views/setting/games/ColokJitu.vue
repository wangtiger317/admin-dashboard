<template>
  <b-card-code>
    <b-form @submit.prevent>
      <h3 class="mt-3 ml-3">Colok Jitu Setting</h3>
      <b-row class="mx-2 mt-3" v-for="data in dataSettings" :key="data">
        ...
        <!-- Win Kop (x) -->
        <b-col cols="2">
          <b-form-group label="Win Kop (x)">
            <b-form-input
              v-model="data.win_kop"
              id="number"
              class="form-control"
              :raw="false"
              :options="options.number"
              @keypress="onlyNumber"
              placeholder="Enter Win Kop"
            />
          </b-form-group>
        </b-col>
        ...
        <!-- Win Kepala (x) -->
        <b-col cols="2">
          <b-form-group label="Win Kepala (x)">
            <b-form-input
              v-model="data.win_kepala"
              id="number"
              class="form-control"
              :raw="false"
              :options="options.number"
              @keypress="onlyNumber"
              placeholder="Enter Win Kepala"
            />
          </b-form-group>
        </b-col>
        ...
        <!-- Win Ekor (x) -->
        <b-col cols="2">
          <b-form-group label="Win Ekor (x)">
            <b-form-input
              v-model="data.win_ekor"
              id="number"
              class="form-control"
              :raw="false"
              :options="options.number"
              @keypress="onlyNumber"
              placeholder="Enter Win Ekor"
            />
          </b-form-group>
        </b-col>
      ...
      <b-col class="mb-3 mt-2 ml-2">
        <b-button
          ...
          variant="primary"
          class="mr-1"
          @click="updateJitu('success', 'top-center', dataSettings)"
        >
          Submit Settings
        </b-button>
      </b-col>
...
export default {
  ...
  methods: {
    ...mapActions('settings', ['updateColokJitu']),
    // number only
    onlyNumber($event) {
    ...
    async updateJitu(variant, position, item) {
      ...
      if (res.value) {
        if (await this.updateColokJitu(item)) {
          this.$toast(
            {
