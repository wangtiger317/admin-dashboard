<template>
  <b-card-code>
    <b-form @submit.prevent>
      <h3 class="mt-3 ml-3">Colok Naga Setting</h3>
      ...
      <b-col class="mb-3 mt-2 ml-2">
        <b-button
          ...
          variant="primary"
          class="mr-1"
          @click="updateNaga('success', 'top-center', dataSettings)"
        >
          Submit Settings
        </b-button>
      </b-col>
...
export default {
  ...
  methods: {
    ...mapActions('settings', ['updateColokNaga']),
    // number only
    onlyNumber($event) {
    ...
    async updateNaga(variant, position, item) {
      ...
      if (res.value) {
        if (await this.updateColokNaga(item)) {
