<template>
  ...
  >
    <template #default="{ hide }">
      ...
      >
        <h5 class="mb-0">Create Free Bet</h5>
        <feather-icon
...
export default {
  ...
  methods: {
    async flipIn(variant, position) {
      ...
      if (result.value) {
        if (await this.$store.dispatch('bonus/createFree', this.memberData)) {
          this.$toast(
            {
